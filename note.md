...
#### Projection Method
This is to resolve the position when overlapping
...
#### Impluse Method
This is to transfer the impluse and speed etc etc

Momentum is a measurement of __mass__ in motion
```
P_m = m * v
```
Conservation of momentum, the total momentum of a system should be constant.

As object collide, they impart some of their momentum on the other object, reducing their own.
...
Impulse:
- a change in momentum by performing a direct change in velocity
- the impulse is generated by the velocity at which the two bodies collided
- relative velocity 
```
v_rel = v_a - v_b
v_rel * Normal
...
```
F * delta_t = delta_P = m * v' - m * v
F * delta_t = m * a * delta_t 
F * delta_t = m * (delta_v / delta_t) * delta_t
F * delta_t = m * delta_v 
```
```cpp
...
void Collision::ResolveCollision(Contact& contact) {
  ...
  Vec2 jn = impulseMagnitude * impulseDirection;
